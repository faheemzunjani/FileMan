...
/**
A File Manager with integrated Text Editor.  

Includes support classes FileTableModel & FileTreeCellRenderer.

@TODO Bugs
<li>Still throws occasional AIOOBEs and NPEs
    ...
    public Container getGui() {
        if (gui==null) {
            ...
            editFile.addActionListener(new ActionListener(){
                public void actionPerformed(ActionEvent ae) {
                    try {
                        char[] fname = (currentFile.getName()).toCharArray();
                        int flen = fname.length;
                        if(fname[flen-1]=='t'&&fname[flen-2]=='x'&&fname[flen-3]=='t'){
                            TextEditor editor = new TextEditor();
                            editor.setVisible(true);
                            File edfile = currentFile;
                            FileReader reader1 = null;
                            try {
                                reader1 = new FileReader(edfile);
                                editor.textComp.read(reader1, null);
                            }
                            ...
                            showErrorMessage("Not an Text File","Edit Failed");
                    } catch(Throwable t) {
                        
                        showErrorMessage("Not an Editable File","Edit Failed");
                    }
                }
            });
